...
class HomebaseController extends AppframeController {
	...
	function _initialize() {
		...
		$live_ids = explode(",", $live_show_ids);
		//遍历
		foreach($live_ids as $live_id){
			$where1['id'] = array("in", $live_id);
			$where2['uid'] = array("in", $live_id);
			$live_show=M("users")
			->field("id,description,user_nicename,avatar")
			->where($where1)
			->limit(30)
			->select();
			$live_show_living=M("users_live")
			->field("uid,avatar,user_nicename,thumb,stream,pull,title,city,islive, topicid")
			->where($where2)
			->limit(30)
			->select();
			foreach($live_show as $k=>$vo){
				...
				foreach($live_show_living as $l=>$lo){
					if($vo['id']==$lo['uid']){
						if($lo['islive']=='1'){
							$live_show[$k]['islive']='1';
							$live_show[$k]['pull']=$lo['pull'];
						}
						else{
							$live_show[$k]['islive']='0';
							$live_show[$k]['pull']=null;
						}
					}
				}
				array_push($live_show_result, $live_show[$k]);
			}
		}
		$this->assign("live_show",$live_show_result);
		
		//回放用户
		$live_record_ids=$config['live_record'];
		$record_ids = explode(",",$live_record_ids);
		$record_results = [];
		//遍历
		foreach($record_ids as $live_id){
			$where1['id'] = array("in", $live_id);
			$live_record=M("users")
			...
			array_push($record_results, $live_record[$k]);
		}
		$this->assign("live_record",$record_results);
		//轮播
		$slides=M("slide")
