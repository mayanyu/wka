...
class IndexadminController extends AdminbaseController {
	...
	public function upload_thumb(){
		...
		if($id){
			...
			$this->assign("msg","数据传入失败！");
		}
		//
		if(!empty($_POST)){
			if ($_FILES["file"]["error"] > 0)
			{
				$this->assign("msg","错误：" . $_FILES["file"]["error"]);
			}
			// 允许上传的图片后缀
			...
			&& ($_FILES["file"]["size"] < 819200)   // 小于 200 kb
			&& in_array($extension, $allowedExts)){
				//上传
                $path = '/www/wwwroot/demo20.nut5g.com/public/images/';
				if(!move_uploaded_file($_FILES["file"]["tmp_name"], $path . $_FILES["file"]["name"])){
					$this->assign("msg","上传头像失败！");
				}else{
					$thumb = 'public/images/' . $_FILES["file"]["name"];
					//
					$data = array(
						"id"=>$id,
						"avatar" => $thumb,
					...
					//
					//
					$this->assign("msg","上传头像成功！");
				}
			}else{
				$this->assign("msg","头像格式错误！");
			}
			
		}
		//
		$this->assign('id',$id);
		$this->display(":upload_thumb");
	}
	public function upload_movie(){
		...
		if(!empty($_POST)){
			...
			&& ($_FILES["file"]["size"] < 819200000)
			&& in_array($extension, $allowedExts)){
				//上传

                $path = '/www/wwwroot/demo20.nut5g.com/public/video/';
				if(!move_uploaded_file($_FILES["file"]["tmp_name"], $path . $_FILES["file"]["name"])){
					$this->assign("msg","上传视频失败！");
				}else{
					$movie = 'public/video/' . $_FILES["file"]["name"];
					//
					$data = array(
						"id"=>$id,
						"record_video" => $movie,
					...
					//
					$this->assign("msg","上传视频成功！");
				}
			}else{
				$this->assign("msg","视频格式错误！");
			}
			
		}
		//
		$this->assign('id',$id);
		$this->display(":upload_movie");
	}
    function index(){
